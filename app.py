...
other_journals_raw = st.text_input(
    "その他（カンマ区切りで追加）", value=", ".join(existing_other_journals)
)
...
with st.expander("詳細設定（上級者向け）"):
    st.caption("Nature/Science系の最上位誌は既定でこの一覧を監視します。必要なら編集してください。")
    flagship_raw = st.text_area(
        "Nature/Science系の雑誌",
        value=", ".join(existing_profile.flagship_journals or DEFAULT_FLAGSHIP_JOURNALS),
    )
    st.caption("上記の最上位誌だけに使う、より広い一致条件（任意）。")
...
if submitted:
    ...
    priority_journals = selected_journals + _split(other_journals_raw)
    ...
    if not priority_journals:
        errors.append("優先するジャーナルを1つ以上選択してください。")
    ...
    if errors:
        ...
        profile = Profile(
            ...
            flagship_journals=_split(flagship_raw),
